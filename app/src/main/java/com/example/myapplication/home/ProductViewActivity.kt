    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(wishlistProductId!=null){
            ...
            binding.addToWishlist.setOnClickListener{
                Snackbar.make(binding.snackbarView,"Removing from wishlist...",Snackbar.LENGTH_SHORT).setAnchorView(binding.addToCartClick).show()
                if(wishlistProductId!=null){
                removeFromWishlist(wishlistProductId.toString())
            }
                else
                {
                    Snackbar.make(binding.snackbarView,"Adding to wishlist...",Snackbar.LENGTH_SHORT).setAnchorView(binding.addToCartClick).show()
                    addToWishlist(pID)
                }
        }
        if(cartItemId!=null){
    ...
    private fun removeFromWishlist(wishlistProductId2: String) {
        ...
        dat.observe(this) {
            if(it==200){
                ...
                Snackbar.make(binding.snackbarView,"Item removed from wishlist",Snackbar.LENGTH_SHORT).setAnchorView(binding.addToCartClick).show()
            }
            else{
                Snackbar.make(binding.snackbarView,"Failed to remove item from wishlist",Snackbar.LENGTH_SHORT).setAnchorView(binding.addToCartClick).show()
            }
        }
    }
    private fun addToWishlist(pid: String?){
        ...
        data.observe(this){
            if(it!="fail"){
                ...
                binding.addToWishlist.setImageResource(R.drawable.wishlisted)
                Snackbar.make(binding.snackbarView,"Item added to wishlist",Snackbar.LENGTH_SHORT).setAnchorView(binding.addToCartClick).show()
            }
            else{
                Snackbar.make(binding.snackbarView,"Failed to add item to wishlist",Snackbar.LENGTH_SHORT).setAnchorView(binding.addToCartClick).show()
            }
        }
    }
        ...
        callApi.enqueue(object : retrofit2.Callback<AddtoCartResponse>{
            override fun onResponse(
                ...
                if(response.code()==201){
                    ...
                    Snackbar.make(binding.snackbarView,"Added to cart successfully!",Snackbar.LENGTH_SHORT).setAnchorView(binding.addToCartClick).show()
                }
                else if(response.code()==400){
                    Snackbar.make(binding.snackbarView,response.body()!!.msg.toString(),Snackbar.LENGTH_SHORT).setAnchorView(binding.addToCartClick).show()
                    binding.addToCartClick.isEnabled=true
                }
            }
            override fun onFailure(call: Call<AddtoCartResponse>, t: Throwable) {
                binding.addToCartClick.isEnabled=true
                Snackbar.make(binding.snackbarView,"Some error occurred",Snackbar.LENGTH_SHORT).setAnchorView(binding.addToCartClick).show()
            }
        })
    }
}